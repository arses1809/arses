...
## Split deployment: Panel + Node

This release supports two runtime roles using `RVG_ROLE`:

- `panel`: central management/UI/API. Proxy transport routes and local MTProto/proxy workers are disabled.
- `node`: actual protocol worker. VLESS/Trojan/Shadowsocks, XHTTP, MTProto and the node management API are enabled.

For Railway, set `RVG_ROLE=panel`. Deploy one or more nodes on providers that permit the intended workload with `RVG_ROLE=node`, then create a node key on the node and connect it from the panel.

See `PANEL_NODE_DEPLOY.md` for the complete setup.
